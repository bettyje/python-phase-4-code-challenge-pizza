...
def resturants_by_id(id):
    ...
    if restaurants:
        if request.method == "GET":
            response_body = restaurants.to_dict(rules=(
                '-restaurant_pizzas.pizza.restaurant_pizzas', '-restaurant_pizzas.restaurant'))
            return make_response(response_body, 200)
...
def all_restaurant_pizza():
    ...
    try:
        new_restaurant_pizza = RestaurantPizza(
            price=price_data, pizza_id=pizza_id_data, restaurant_id=restaurant_id_data)
